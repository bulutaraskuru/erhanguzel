...
class indexController extends Controller
{
    ...
    public function store(ProductCategoryRequest $request)
    {
        ...
        $img800x600 = imageHelper::upload800x600(uniqid(), 'productcategory', '800x600', $all['img']);
    ...
    {
        $all = $request->except('_token');
        $old_data = ProductCategory::findOrFail($id);

        if (isset($all['img'])) {
            $img800x600 = imageHelper::upload800x600(uniqid(), 'productcategory', '800x600', $all['img']);
            \File::delete(public_path($old_data->img));
            $updateImage = ProductCategory::where('id', $id)->update([
    ...
    {
        $control = ProductCategory::where('id', $id)->count();
        if ($control != 0) {
            $old_data = ProductCategory::findOrFail($id);
            \File::delete(public_path($old_data->img));
    ...
    public function upload(Request $request)
    {
        ...
        \Image::make($mainImage)->save(public_path('images/productcategory/content_image/'.$fileName));

        return response()->json(['location' => url('/images/productcategory/content_image/'.$fileName)]);
    }
}
